...
def get_all_EERs(
    ...
    fpr, tpr, thres_sasv = roc_curve(sasv_labels, preds, pos_label=1)
    sasv_eer = brentq(lambda x: 1.0 - x - interp1d(fpr, tpr)(x), 0.0, 1.0)
    thres_sasv = thres_sasv[np.nanargmin(np.abs(fpr - (1 - tpr)))]

    fpr, tpr, thres_sv = roc_curve(sv_labels, sv_preds, pos_label=1)
    sv_eer = brentq(lambda x: 1.0 - x - interp1d(fpr, tpr)(x), 0.0, 1.0)
    thres_sv = thres_sv[np.nanargmin(np.abs(fpr - (1 - tpr)))]

    fpr, tpr, thres_spf = roc_curve(spf_labels, spf_preds, pos_label=1)
    spf_eer = brentq(lambda x: 1.0 - x - interp1d(fpr, tpr)(x), 0.0, 1.0)
    thres_spf = thres_spf[np.nanargmin(np.abs(fpr - (1 - tpr)))]
    

    return sasv_eer, sv_eer, spf_eer

def get_all_EERs_new(
    preds: Union[torch.Tensor, List, numpy.ndarray], keys: List
) -> List[float]:
    """
    Calculate all three EERs used in the SASV Challenge 2022.
    ...
    return sasv_eer, sv_eer, spf_eer, thres_sasv, thres_sv, thres_spf
