...
class ParallelModel(nn.Module):
    def __init__(self, cm_input_dim, cm_hidden_layers):
    ...
    def forward(self, asv_score, cm_input):
        cm_output = self.cm_classifier(cm_input)
        asv_out = self.asv_layer(asv_score)
        cm_out = self.cm_layer(cm_output)
        shift = (asv_out + cm_out) / 2.0
        # Compute s_sasv
        s_sasv = (
            self.rho * torch.exp(-asv_out + shift)
            + (1 - self.rho) * torch.exp(-cm_out + shift)
        )
        epsilon = 1e-10
...
def train(model, dataloader, optimizer, loss_fn, loss_adcf, device):
    ...
    for batch in dataloader:
        ...
        asv_scores = F.cosine_similarity(asv1, asv2, dim=1)
        cm_inputs = torch.cat((asv2, cm1), dim=1)
        targets = labels.to(device).float()

        with torch.enable_grad():
            optimizer.zero_grad()  # Reset gradients
            predictions, asv_out, cm_out = model(asv_scores.reshape(-1,1), cm_inputs)  # Forward pass
    ...
    return total_loss / len(dataloader), l_adcf / len(dataloader), l_bce / len(dataloader), \
...
def evaluate(model, dataloader, device):
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            cm_inputs = torch.cat((asv2, cm1), dim=1)
            targets = labels.to(device).float()
            predictions, asv_out, cm_out = model(asv_scores.reshape(-1,1), cm_inputs)
...
if __name__ == "__main__":
    ...
    def format_log_message(epoch, trn_metrics, dev_metrics):
        trn_loss, trn_loss_adcf, trn_loss_bce, trn_loss_cm, trn_sasv_eer, trn_sv_eer, trn_spf_eer, \
        ...
        return (
            ...
            f"Train Loss CM: {trn_loss_cm:.4f}, "
    ...
    def print_final_results(eval_metrics):
        ...
        print("\n=== Random Non-Linear Fusion L2 FrozenASV ===")
    ...
    def save_kde_plot(best_preds, best_keys):
        ...
        plt.ylabel("Density")
        plt.legend()
        final_kde_path = os.path.join(args.output_dir, "dev_scores_kde.png")
